...
const connectCeloWallet = async function () {
  if (window.ethereum) {
    try {
      ...
      cUSD_spender_contract = new kit.web3.eth.Contract(
        cUSD_spender.abi,
        cUSD_spender.networks[chainId].address
      );
    } catch (error) {
      console.log(`⚠️ ${error}.`);
    }
...
async function approve(amount) {
  const result = await cUSDcontract.methods
    ...
    .approve(
      cUSD_spender_contract._address,
...
async function send(recipient, amount, billing_code) {
  const result = await cUSD_spender_contract.methods
    // Call the "send" function on the cUSD_spender contract
    .send(recipient, kit.web3.utils.toWei(amount, "ether"), billing_code)
    .send({ from: kit.defaultAccount });
...
document.querySelector("#send").addEventListener("click", async (e) => {
  ...
  let place_of_service = document.getElementById("place_of_service").value;
  let emergency = document.getElementById("emergency").value;
  let CPT_code = document.getElementById("CPT_code").value;
  let diagnosis_pointer = document.getElementById("diagnosis_pointer").value;
  let epsdt = document.getElementById("epsdt").value;
  let providor_id_NPI = document.getElementById("providor_id_NPI").value;
  let str = "Date of Service:   " + date_of_service;
  str += "\nPlace of Service:  " + place_of_service;
  str += "\nEmergency:         " + emergency;
  str += "\nCPT Code:          " + CPT_code;
  str += "\nDiagnosis Code:    " + diagnosis_pointer;
  str += "\nEPSDT:             " + epsdt;
  str += "\nProvidor ID (NPI): " + providor_id_NPI;
  send(recipient, amount, str);
});
